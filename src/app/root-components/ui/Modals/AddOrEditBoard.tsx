...
import { getAddOrEditBoardModalValue, closeAddOrEditBoardModal, getPageTitle, 
...
interface IAddBoardData {
    name: string,
    columns: { name?: string, columns?: { name: string, tasks?: {[ key: string]: any}[]}}[]
}
...
export default function AddOrEditBoardModal() {
    ...
    const handleColumnNameChange = (name: string) => { 
        return function (e: React.ChangeEvent<HTMLInputElement>) {
            // handle change for create new board modal
             if(boardData) {
                     const modifyColumns = boardData.columns.map((column: { name?: string}) => {
                       if(column.name === name) {
    ...
    const handleDeleteColumn = (index: number) => {
        if(boardData) {
            const filteredColumns = boardData.columns.filter((column, columnIndex) => columnIndex !== index)
    ...
    const handleEditBoard = () => {
        const emptyStringChecker = boardData?.columns.some(column => column.name === '')
        if(boardData?.name !== '' && !emptyStringChecker) {
            if(data) {
              const [ boards ] = data;
              const boardsCopy = [ ...boards.boards ] // create a copy of the data that is not read-only
              const activeBoardIndex = boardsCopy.findIndex((board: {name: string}) => board.name === currentBoardTitle)
              const updatedBoard = {
            ...
            <ModalBody>
                ...
                <div className='py-6'>
                    ...
                    <div className="pt-6">
                        ...
                        {  boardData && (
                            boardData.columns.map((column: {name?: string,}, index: number) => {             
                            let { name } = column 
                            return (
                        ...
                        <div className='pt-3'>
                        <Button onClick={handleAddNewColumn} intent='primary' text={isVariantAdd ? '+ Add New Board': '+ Add New Column'}/>
                        </div>
                    </div>
                    <div className="pt-6">
                     <Button onClick={() => {isVariantAdd ? handleAddNewBoardToDb() : handleEditBoard()}} intent='secondary' text={isVariantAdd ? 'Create New Board': 'Save Changes'}/>
                    </div>
                  </div>
                </>
                )}
